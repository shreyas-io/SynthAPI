...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`
    ...
    create table organization_plan_subscriptions (
      id uuid primary key default uuidv7(),
      organization_id uuid not null references organizations(id) on delete cascade,
    ...
    create unique index organization_plan_subscriptions_one_active_idx
    ...
    create table organization_credit_usages (
      id uuid primary key default uuidv7(),
      organization_id uuid not null references organizations(id) on delete cascade,
    ...
    create index organization_memberships_status_idx on organization_memberships(organization_id, status);
    create index organization_plan_subscriptions_plan_type_id_idx on organization_plan_subscriptions(plan_type_id);
    create index organization_plan_subscriptions_expires_at_idx on organization_plan_subscriptions(expires_at);
    create index organization_credit_grants_org_type_expiry_idx on organization_credit_grants(organization_id, grant_type, expires_at);
    create index organization_credit_grants_subscription_idx on organization_credit_grants(source_subscription_id);
    create index organization_credit_usages_grant_idx on organization_credit_usages(credit_grant_id);
    create index organization_credit_usages_org_created_idx on organization_credit_usages(organization_id, created_at);
